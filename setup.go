...
func (pp *PublicParams) Paramgen(N int) {
	...
	pp.pp_generators_alpha_beta = make([]mcl.G1, 2*pp.n*pp.n)
	...
	for i := 0; i < pp.n; i++ {
		...
		for j := 0; j < pp.n; j++ {
			...
			mcl.G1Mul(&pp.pp_generators_alpha_beta[i*(2*pp.n)+j], &pp.g1, &alpha_beta_power)
		}
		//skip g1^{alpha^i beta^{n+1}}
		mcl.FrMul(&alpha_beta_power, &alpha_beta_power, &beta)
		pp.pp_generators_alpha_beta[i*(2*pp.n)+pp.n].SetString("0", 10)
